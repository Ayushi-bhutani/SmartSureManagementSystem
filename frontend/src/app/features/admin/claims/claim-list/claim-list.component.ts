...
@Component({
  ...
  template: `
    <div class="admin-claim-list-page">
      ...
      <div class="container">
        ...
        <div class="claims-grid" *ngIf="filteredClaims.length > 0">
          <mat-card *ngFor="let claim of filteredClaims" class="claim-card">
            ...
            <div class="claim-actions">
              <button mat-raised-button color="primary" (click)="reviewClaim(claim.claimId)"
                      *ngIf="claim.status === 'Submitted' || claim.status === 'Under Review'">
        ...
        <div class="empty-state" *ngIf="filteredClaims.length === 0 && !isLoading">
          ...
          <p *ngIf="!searchTerm && !statusFilter">No claims have been submitted yet</p>
        </div>
      </div>
...
export class AdminClaimListComponent implements OnInit {
  ...
  loadClaims(): void {
    this.isLoading = true;
    this.claimService.getAllClaims(1, 100).subscribe({
      next: (response) => {
